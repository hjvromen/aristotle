...
/- We start with a syntactical account of Aristotle's 
   assertoric (non-modal) syllogistic. 
   After that, three differrent semantics are presented 
   with a proof of soundness of the proof system.
-/

import aristotle_language
...
constant a (A : term) (B : term) : Prop    -- universal_affirmation
local infixr ` a ` : 80 := a

constant e (A : term) (B : term) : Prop     -- universal_denial
local infixr ` e ` : 80 := e

constant i (A : term) (B : term) : Prop     -- particular_affirmation
local infixr ` i ` : 80 := i

constant o (A : term) (B : term) : Prop     -- particular_denial
local infixr ` o ` : 80 := o
...
/- Aristotle also argues that Darii and Ferio do not have to be 
   assumed as axioms, but can be derived from the other axioms -/

lemma Darii_deduction (h1 : P a M) (h2 : M i S) : P i S :=
begin
...
lemma Ferio_deduction (h1 : P e M) (h2 : M i S) : P o S :=
begin
...
/- 
so, Barbara and Celarent and e-conv and a-conv and the rules for
   the contradictories suffice to prove all 24 valid syllogisms; we call this 
   system DR
-/

/-
************** TODO ****************
1. prove that all invalid syllogisms cannot be derived
...
b. is it possible to prove these rules in our proof system?

2. give an inductive definition of a syllogism, maybe starting with:
